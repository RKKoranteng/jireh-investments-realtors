...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'jirehrealtors'),
        'collapse' => __('collapse child menu', 'jirehrealtors'),
    ));

    // remove plugin's scripts and styles
    wp_dequeue_style("jirehrealtor-style");
    wp_dequeue_style("jirehrealtor-responsive");
    wp_dequeue_style("jirehrealtor-media");
    wp_dequeue_script("jirehrealtor-script");

    if (theme_get_option('include_jquery')) {
        wp_dequeue_script("jirehrealtor-jquery");
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'jirehrealtors'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'jirehrealtors'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'jirehrealtors');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'jirehrealtors'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'jirehrealtors'));
}
...
remove_action('wp_footer', 'Jirehrealtor::wpFooterAction', 1001);
...
add_filter('jirehrealtor_template', 'theme_app_template');
