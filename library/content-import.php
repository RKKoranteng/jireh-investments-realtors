...
class ThemeContentImport {
    ...
    public static function contentImportNoticeAction() {
        global $tgmpa;
        $import_content_can = has_action('jirehrealtor_import_content');
        ...
        <div id="content-import-notice" class="updated">
            <p>
                <?php echo __('Do you want to import Content?', 'jirehrealtors'); ?>
                &nbsp; &nbsp; &nbsp; &nbsp;
                <a id="import-import-content" class="import-button button" href="<?php echo $import_href; ?>"><?php echo __('Import content', 'jirehrealtors'); ?></a>
                <a id="import-replace-content" class="<?php if (get_option('np_imported_content') === false) echo 'hidden'; ?> import-button button" href="<?php echo $import_href; ?>"><?php echo __('Replace previously imported content', 'jirehrealtors'); ?></a>
                <a id="import-hide-notice" class="import-button button" href="#"><?php echo __('Hide notice', 'jirehrealtors'); ?></a>
            </p>
        </div>
        <style>
            .import-button {
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                function bindImportAction(action, btn) {
                    ...
                    var successMsg;
                    var captchaNotice = "<p>Keys for <strong>recaptcha</strong> replaced. If necessary, you can change keys manually in 'Site Settings' in the <strong>JirehRealtor</strong> plugin</p>";
                    var import_content_can = "<?php echo $import_content_can; ?>";
                    var failMsg = <?php echo json_encode(__('An error occurred while importing.', 'jirehrealtors')); ?>;
                    var captchaKeys;
                    btn.unbind("click").click(function() {
                        ...
                        doAjax(action).done(function (response) {
                            ...
                            if (import_content_can) {
                                successMsg = <?php echo json_encode(__('Content was successfully imported.', 'jirehrealtors')); ?>;
                                $('#content-import-notice').html('<p>' + successMsg + '</p>' + captchaMsg);
                            } else {
                                successMsg = <?php echo json_encode(__('Please install and activate <a href="themes.php?page=tgmpa-install-plugins&plugin_status=install">JirehRealtor plugin</a> for import content.', 'jirehrealtors')); ?>;
    ...
    public static function pluginRequiredNoticeAction() {
        ...
        <div id="content-import-notice" class="notice notice-warning">
            <p><?php _e('Import process required plugin to be active.', 'jirehrealtors'); ?></p>
            <p><?php $tgmpa->is_plugin_installed('jirehrealtor')
                    ? _e('Please activate plugin to continue.', 'jirehrealtors')
                    : _e('Please install and activate plugin to continue.', 'jirehrealtors'); ?></p>
        </div>
        <?php
    }
    ...
    private static function _importData($remove_prev = false) {
        ...
        do_action('jirehrealtor_import_content', $content_dir, $remove_prev);
    }
