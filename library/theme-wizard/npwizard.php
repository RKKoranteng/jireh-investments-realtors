...
class Npwizard {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'verify_text'    => esc_html('verifying', 'jirehrealtors'),
    ...
    public function menu_page() {
        add_theme_page(esc_html__('Theme Wizard', 'jirehrealtors'), esc_html__('Theme Wizard', 'jirehrealtors'), 'manage_options', $this->page_slug, array($this, 'wizard_page'));
    }
    ...
    public function get_steps() {
        ...
        $steps = array(
            'plugins' => array(
                'id'			=> 'plugins',
                'title'			=> __('Install Theme Plugins', 'jirehrealtors'),
                'icon'			=> 'admin-plugins',
                'view'			=> 'get_step_plugins',
                'callback'		=> 'install_plugins',
                'button_text'	=> __('Install Plugins', 'jirehrealtors'),
                'can_skip'		=> false
            ),
            'content' => array(
                'id'			=> 'content',
                'title'			=> __('Import Content', 'jirehrealtors'),
                'icon'			=> 'welcome-content-menus',
                'view'			=> 'get_step_content',
                'callback'		=> 'import_content',
                'callback2'		=> 'replace_content',
                'button_text'	=> __('Import Content', 'jirehrealtors'),
                'button2_text'	=> __('Replace previously imported Content', 'jirehrealtors'),
                'can_skip'		=> true,
                'can_replace'	=> true
            ),
            'done' => array(
                'id'			=> 'done',
                'title'			=> __('Your website is ready!', 'jirehrealtors'),
                'icon'			=> 'yes',
                'view'			=> 'get_step_done',
                'callback'		=> ''
            )
    ...
    public function get_step_plugins() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Install plugins included with the Theme. <br>Click the "Install Plugins" button to start the installation.', 'jirehrealtors')
        );
    ...
    public function get_step_content() {
        ...
        if($hideImport) {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Content has already been imported. Please skip this step', 'jirehrealtors')
           );
        } else {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Theme has Pages, Images, Menu, Header, and Footer. </br></br>Do you want to import the Content?', 'jirehrealtors')
            );
        }
    ...
    public function get_step_done() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Congratulations! The theme has been activated and your website is ready.', 'jirehrealtors')
        );
        $content['summary'] .= sprintf( '<p>%s</p>', 'Create a new page with the JirehRealtor Editor.', 'jirehrealtors' );
	...
	public function setup_plugins() {
		if (! check_ajax_referer('npwizard_nonce', 'wpnonce') || empty($_POST['slug'])) {
			wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'jirehrealtors')));
		}
		$json = array();
		...
		foreach ($plugins['activate'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Activating Plugin', 'jirehrealtors'),
		...
		foreach ($plugins['update'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Updating Plugin', 'jirehrealtors'),
				);
				break;
			}
		}
		foreach ($plugins['install'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Installing Plugin', 'jirehrealtors'),
				);
				break;
			}
		}
		if ($json) {
			...
			wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'jirehrealtors')));
		}
		exit;
	}
    ...
    public function setup_content() {
        ...
        $json = array(
            ...
            'message'       => esc_html__('Import Content', 'jirehrealtors'),
       );
        if ($json) {
            $json['hash'] = md5(serialize($json)); // used for checking if duplicates happen, move to next plugin
            wp_send_json($json);
